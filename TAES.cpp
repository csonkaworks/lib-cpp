// This is an OOP version of the code written by author described below.
// reWritten by Gabor Csonka, 2009. Comments suggestions and donations are
// welcome. E-mail address is g.csonka@freemail.hu
// Also, this is the address you can send donations via PayPal.
...
memcpy(&Rcon,&Rcon_tmp,255);
}

int TAES::getSBoxValue(int num)
{
	int sbox[256] =   {
...
void TAES::AddRoundKey(int round)
{
	int i,j;
	for(i=0;i<4;i++)
	{
...
void TAES::Cipher(int keylength)
{
	int i,j,round;
...
int TAES::getSBoxInvert(int num)
{
int rsbox[256] =
...
void TAES::InvCipher(int keylength)
{
	int i,j,round;
